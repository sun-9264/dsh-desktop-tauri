...
用 **Tauri 2.x（Rust + 系统 WebView2）** 重写的 DeepSeek Harness 第三方桌面启动器，把体积从 Electron 版的 ~71MB 降到 **~9.7MB**（单文件 exe）/**2.11MB**（NSIS 安装包），内存占用更低。
...
2. **主题/插件装配**：`--profile web` 会加载你 `~/.dsh` 装配的插件/主题；但要"开箱自带"需把装配打包进应用（待做）。
...
| **Tauri 版** | ~9.7MB（exe）/ 2.11MB（NSIS 安装包） | 复用系统 WebView2 |
